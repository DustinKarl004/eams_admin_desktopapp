...
async function fetchApplicantData(searchTerm = '') {
    try {
        ...
        const applicantFormSnapshot = await getDocs(collection(db, 'freshmen_applicant_form'));
        ...
        if (searchTerm) {
            ...
            applicants = applicants.filter(applicant => {
                // Handle suffix display
                const suffixDisplay = applicant.suffix ? ` ${applicant.suffix}` : '';
                
                // Format different name combinations
                const formalFormat = `${applicant.last_name}, ${applicant.first_name} ${applicant.middle_name}${suffixDisplay}`.toLowerCase();
                const naturalFormat = `${applicant.first_name} ${applicant.middle_name} ${applicant.last_name}${suffixDisplay}`.toLowerCase();
                const shortFormat = `${applicant.first_name} ${applicant.last_name}${suffixDisplay}`.toLowerCase();
                const lastFirstFormat = `${applicant.last_name} ${applicant.first_name}${suffixDisplay}`.toLowerCase();
                const lastFirstMiddleFormat = `${applicant.last_name} ${applicant.first_name} ${applicant.middle_name}${suffixDisplay}`.toLowerCase();
        ...
        card.innerHTML = `
            <div class="applicant-card">
                <h4 class="applicant-name">
                    ...
                    ${applicant.last_name}, ${applicant.first_name} ${applicant.middle_name}${suffixDisplay}
                </h4>
                ...
                <div class="applicant-actions">
                    <a href="admin_freshmen_alldataofapplicant.html?user_id=${encodeURIComponent(applicant.id)}" class="btn btn-see-more">See More</a>
                    ${applicant.status === 'APPROVED' ? `<a href="admin_freshmen_checkstatus.html?user_id=${encodeURIComponent(applicant.id)}" class="btn btn-check-status">Check Status</a>` : ''}
                </div>
            </div>
        `;
