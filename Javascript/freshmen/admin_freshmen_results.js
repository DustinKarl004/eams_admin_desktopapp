...
async function populateIdSelect() {
    ...
    const resultsSnapshot = await getDocs(collection(db, 'freshmen_examinees_result'));
...
selectId.addEventListener('change', async () => {
    ...
    if (selectedId) {
        const examineeDoc = await getDoc(doc(db, 'freshmen_examinees', selectedId));
...
window.addResult = async function() {
    ...
    if (id && email && fullName && institute && course) {
        try {
            await setDoc(doc(db, 'freshmen_examinees_result', id), {
                email,
                fullName,
                institute,
                course
...
window.updateResult = async function() {
    ...
    if (id && email && fullName && institute && course) {
        try {
            await updateDoc(doc(db, 'freshmen_examinees_result', id), {
                email,
                fullName,
                institute,
                course
...
async function deleteResult(id, email) {
    try {
        ...
        const assessmentQuery = query(collection(db, 'freshmen_assessment'), where('email', '==', email));
        const assessmentSnapshot = await getDocs(assessmentQuery);

        // Check if there's an enrollment record for this email
        const enrollmentQuery = query(collection(db, 'freshmen_enrollment'), where('email', '==', email));
        const enrollmentSnapshot = await getDocs(enrollmentQuery);

        // Check if there's a medical completion record for this email
        const medicalQuery = query(collection(db, 'freshmen_medical_completion'), where('email', '==', email));
        ...
        await deleteDoc(doc(db, 'freshmen_examinees_result', id));
        deleteConfirmModal.hide();
        fetchResults();
