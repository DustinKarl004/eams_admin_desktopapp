...
async function fetchApplicantData(userId) {
    try {
        ...
        const docRef = doc(db, 'freshmen_applicant_form', userId);
...
function renderPersonalInfo(data) {
    ...
    const suffixDisplay = info.suffix ? ` ${info.suffix}` : '';
    
    return `
        <div class="section-title">
            <i class="fas fa-user-circle me-2"></i>Personal Information
        </div>
        <p class="info-item"><i class="fas fa-user"></i>${info.last_name || ''}, ${info.first_name || ''} ${info.middle_name || ''}${suffixDisplay}</p>
...
function renderSectorAndWorkStatus(data) {
    ...
    return `
        ...
        <p class="info-item"><i class="fas fa-industry"></i>${info.sector === 'Other' ? `${info.sector} - ${info.otherSector}` : info.sector || ''}</p>
...
function renderFamilyInfo(data) {
    ...
    // Handle father's suffix display
    const fatherSuffixDisplay = info.father_suffix ? ` ${info.father_suffix}` : '';
    
    return `
        <div class="section-title">
            <i class="fas fa-users me-2"></i>Family Information
        </div>
        <p class="info-item"><i class="fas fa-male"></i>Father: ${info.father_last_name || ''}, ${info.father_first_name || ''} ${info.father_middle_name || ''}${fatherSuffixDisplay}</p>
...
function renderGuardianInfo(data) {
    ...
    // Handle guardian's suffix display
    const guardianSuffixDisplay = info.guardian_suffix ? ` ${info.guardian_suffix}` : '';
    
    return `
        <div class="section-title">
            <i class="fas fa-user-shield me-2"></i>Guardian Information
        </div>
        <p class="info-item"><i class="fas fa-user"></i>${info.guardian_last_name || ''}, ${info.guardian_first_name || ''} ${info.guardian_middle_name || ''}${guardianSuffixDisplay}</p>
        <p class="info-item"><i class="fas fa-home"></i>Residing with Applicant: ${info.is_guardian_residing_on_same_home_address}</p>
    `;
}
...
function renderCapturedImages(data) {
    ...
    return `
        ...
        ${info.uploaded_at ? `
            ...
            Uploaded at: ${new Date(info.uploaded_at).toLocaleString()}            </p>
        ` : ''}
