...
saveApprovedApplicantBtn.addEventListener('click', async () => {
    ...
    if (id && email && fullName) {
        try {
            await setDoc(doc(db, 'transferee_approved_applicants', id), {
                email,
                fullName,
...
async function deleteApprovedApplicant(id, email) {
    try {
        ...
        const examineeQuery = query(collection(db, 'transferee_examinees'), where('email', '==', email));
        ...
        await deleteDoc(doc(db, 'transferee_approved_applicants', id));
        deleteConfirmModal.hide();
        fetchApprovedApplicants();
